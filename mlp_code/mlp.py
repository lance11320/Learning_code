...
def calc_loss_and_grad(x, y, w1, b1, w2, b2, eval_only=False):
    ...
    h1 = utils.relu(z1)
    z2 = np.dot(h1,w2)+b2
    y_hat = utils.softmax(z2)
    loss = utils.cross_entropy_loss(y_hat, y)
    ...
    dz2 = (y_hat-y)/x.shape[0]
    
    dw2 = np.dot(h1.T,dz2)
    db2 = np.sum(dz2,axis = 0)
    
    dh1 = np.dot(dz2,w2.T)
    dz1 = dh1*utils.sgn(z1)
...
def train(train_x, train_y, test_x, text_y, args: argparse.Namespace):
    ...
    w1 = random.randn(input_dim,args.hidden_dim)
    b1 = random.randn(args.hidden_dim)
    w2 = random.randn(args.hidden_dim,output_dim)
    b2 = random.randn(output_dim)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(n_iterations):
            ...
            loss,db2, dw2, db1, dw1 = calc_loss_and_grad(x_batch, y_batch, w1, b1, w2, b2, eval_only=False)
